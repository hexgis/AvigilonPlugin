...
class CameraFinder():
    def __init__(self, canvas, layers_dict, crs):
        ...
        self.cameras_range = self.canvas.layer(
            self.layers_dict['alcance_expansao'])
    ...
    def create_point_aoi(self, position):
        ...
        """
        self.area_of_interest = self.vlm.create_layer('Point', 'buffer_point')
        self.vlm.create_feature(self.area_of_interest, True, position=position)
